...
| ----                                      | -----------                               | :-------: | -------            | -----                       | -----                                                                             |
| APPINSIGHTS_CLOUDROLE                     | Role used for filtering metrics           | no        |                    |                             | Set to `ffc-grants-desirability-notification` in docker compose files                               |
...
| DESIRABILITY_SUBMITTED_SUBSCRIPTION_ADDRESS| queue topic address                      | no        |                    |                             |                                                                                   |
...
`DESIRABILITY_SUBMITTED_SUBSCRIPTION_ADDRESS`, `DESIRABILITY_SUBMITTED_TOPIC_ADDRESS` must be set to a valid, developer specific queue that is
available on ASB e.g. `ffc-grants-desirability-notification-<initials>` where `<initials>` are the
...
kubectl port-forward --namespace=ffc-grants deployment/ffc-grants-desirability-notification 3000:3000
```
